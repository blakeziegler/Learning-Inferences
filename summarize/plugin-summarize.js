  ...
  class SummarizeTextPlugin {
      ...
      trial(display_element, trial) {
          ...
          if (trial.preamble !== null) {
              html +=
                  '<div id="jspsych-summarize-text-preamble" class="jspsych-summarize-text-preamble">' +
                      trial.preamble +
                      "</div>";
          }
          // start form
          if (trial.autocomplete) {
              html += '<form id="jspsych-summarize-text-form">';
          }
          else {
              html += '<form id="jspsych-summarize-text-form" autocomplete="off">';
          }
          ...
          for (var i = 0; i < trial.questions.length; i++) {
			  ...
			  if (i == 0) {
				  html +=
                  '<div id="jspsych-summarize-text-' +
                      question_index +
                      '" class="jspsych-summarize-text-question unobfuscated" style="margin: 2em 0em; max-width: 800px; text-align: justify;">';
			  } else {
				  html +=
                  '<div id="jspsych-summarize-text-' +
                      question_index +
                      '" class="jspsych-summarize-text-question obfuscated" style="margin: 2em 0em; max-width: 800px; text-align: justify;">';
			  }

              html += '<p class="jspsych-summarize-text">' + question.prompt + "</p>";
              ...
              if (i == 0) {
                  html +=
                      '<textarea id="input-' +
                          question_index +
                          '" name="#jspsych-summarize-text-response-' +
                          question_index +
                          '" data-name="' +
                          question.name +
                          '" cols="' +
			  ...
			  html += "</div>";
			  html += '<div id="response-'+question_index+'" class="summarize-response">Explain the text in your own words</div>';
              html += "</div>";
          }
          // add submit button
          html +=
              '<input type="submit" id="jspsych-summarize-text-next" class="jspsych-btn jspsych-summarize-text" value="' +
                  trial.button_label +
...
function getSimilarity(doc1,doc2) {
  // possibility for future implementation https://huggingface.co/tasks/sentence-similarity
...
function checkIt(q) {
  ...
  originaltext = document.getElementById("jspsych-summarize-text-"+thisq).textContent;
  ...
  if (response.length < 10) { // Character length
	  ...
	  addData(document.getElementById("input-"+thisq).attributes["data-name"].value,response,rtime,sim,"error no real words");
	  document.getElementById("response-"+thisq).innerHTML = "Please respond in English words";
	  document.getElementById("response-"+thisq).style.color="red";
	  document.getElementById("input-"+thisq).classList.add('error');
	  setTimeout(function() {
		  document.getElementById("input-"+thisq).classList.remove('error');
	  ...
	  if (simPct < 80) {
		  ...
		  // document.getElementById("response-"+thisq).innerHTML = "Current similarity is " + simPct + "%. Please update your response to reach 80% similarity.";
          document.getElementById("response-"+thisq).innerHTML = "Your summary is missing some key elements. Please be sure to summarize the main ideas in your own words.";
		  document.getElementById("response-"+thisq).style.color="red";
	      document.getElementById("input-"+thisq).classList.add('error');
	  } else {
		  addData(document.getElementById("input-"+thisq).attributes["data-name"].value,response,rtime,sim,"pass");
		  document.getElementById("response-"+thisq).innerHTML = "Similarity: " + simPct + "%";
		  ...
		  if (document.getElementById("jspsych-summarize-text-"+nextq) == null) { // this was the last textarea
			document.getElementById("jspsych-summarize-text-next").disabled = false; // enable the submit button
			document.getElementById("jspsych-summarize-text-next").focus(); // give focus to the submit button
		  } else { 
